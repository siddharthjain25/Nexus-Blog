    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Manage your blog stories and drafts.</p>
        </div>
        <div className="flex w-full sm:w-auto gap-3">
          ...
          <Link to="/admin/new" className="flex items-center justify-center gap-2 bg-primary text-white px-5 py-2.5 rounded-xl hover:bg-blue-600 transition-all shadow-lg shadow-primary/20 font-semibold text-sm">
            <Plus size={18} />
            ...
            {filteredPosts.map(post => (
              <tr key={post.slug} className="hover:bg-slate-800/20 transition-colors group">
                <td className="px-6 py-5">
                  <div className="font-bold text-slate-100 group-hover:text-primary transition-colors">{post.title}</div>
                  <div className="flex items-center gap-1.5 mt-1">
                    <span className="text-xs text-slate-500 font-mono">/{post.slug}</span>
                    <Link to={`/posts/${post.slug}`} target="_blank" className="text-slate-600 hover:text-slate-400 transition-colors">
